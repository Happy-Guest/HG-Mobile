...
import ipl.estg.happyguest.databinding.FragmentFoodBinding;
...
public class FoodFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        binding.btnOrderFood.setOnClickListener(v -> {
        ...
        binding.foodService.btnMenu.setOnClickListener(v -> {
            if (menuURL != null) {
    ...
    private void showPopup() {
        ...
        btnPopConfirm.setOnClickListener(view1 -> {
            ...
            binding.btnOrderFood.setEnabled(false);
            binding.btnHistoryFood.setEnabled(false);
            popupWindow.dismiss();
        });
    }
    ...
    private void getServiceAttempt() {
        Call<ServiceResponse> call = api.getService(3L);
            ...
            public void onResponse(@NonNull Call<ServiceResponse> call, @NonNull retrofit2.Response<ServiceResponse> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (Objects.requireNonNull(service).isActive()) {
                        ...
                        binding.foodService.txtServiceSchedule.setText(schedule);
                        binding.foodService.txtServiceSchedule.setTextColor(getResources().getColor(R.color.black, null));
                    } else {
                        binding.btnOrderFood.setEnabled(false);
                        binding.spinnerRoom.setEnabled(false);
                        binding.inputComment.setEnabled(false);
                        binding.foodService.txtServiceSchedule.setText(R.string.services_unavailable);
                        binding.foodService.txtServiceSchedule.setTextColor(getResources().getColor(R.color.red, null));
                    }
                    if (service.getEmail() != null) {
                        binding.foodService.serviceEmail.setAnimation(AnimationUtils.loadAnimation(binding.getRoot().getContext(), R.anim.fade_in_fast));
                        binding.foodService.serviceEmail.setVisibility(View.VISIBLE);
                        binding.foodService.txtServiceEmail.setText(service.getEmail());
                    } else {
                        binding.foodService.serviceEmail.setAnimation(AnimationUtils.loadAnimation(binding.getRoot().getContext(), R.anim.fade_out_fast));
                        binding.foodService.serviceEmail.setVisibility(View.GONE);
                    }
                    if (service.getPhone() != null) {
                        binding.foodService.servicePhone.setAnimation(AnimationUtils.loadAnimation(binding.getRoot().getContext(), R.anim.fade_in_fast));
                        binding.foodService.servicePhone.setVisibility(View.VISIBLE);
                        binding.foodService.txtServicePhone.setText(String.format(Locale.getDefault(), "%d", service.getPhone()));
                    } else {
                        binding.foodService.servicePhone.setAnimation(AnimationUtils.loadAnimation(binding.getRoot().getContext(), R.anim.fade_out_fast));
                        binding.foodService.servicePhone.setVisibility(View.GONE);
                    }
                    if (service.getOccupation() != null) {
                        binding.foodService.serviceOccupation.setAnimation(AnimationUtils.loadAnimation(binding.getRoot().getContext(), R.anim.fade_in_fast));
                        binding.foodService.serviceOccupation.setVisibility(View.VISIBLE);
                        binding.foodService.txtServiceOccupation.setText(String.format(Locale.getDefault(), "%d", service.getOccupation()));
                    } else {
                        binding.foodService.serviceOccupation.setAnimation(AnimationUtils.loadAnimation(binding.getRoot().getContext(), R.anim.fade_out_fast));
                        binding.foodService.serviceOccupation.setVisibility(View.GONE);
                    }
                    if (service.getLocation() != null) {
                        binding.foodService.serviceLocation.setAnimation(AnimationUtils.loadAnimation(binding.getRoot().getContext(), R.anim.fade_in_fast));
                        binding.foodService.serviceLocation.setVisibility(View.VISIBLE);
                        binding.foodService.txtServiceLocation.setText(service.getLocation());
                    } else {
                        binding.foodService.serviceLocation.setAnimation(AnimationUtils.loadAnimation(binding.getRoot().getContext(), R.anim.fade_out_fast));
                        binding.foodService.serviceLocation.setVisibility(View.GONE);
                    }
                    // Get language code and set description
                    String languageCode = Locale.getDefault().getLanguage();
                    if (languageCode.equals("pt")) {
                        binding.foodService.txtDescription.setText(service.getDescription());
                    } else {
                        binding.foodService.txtDescription.setText(service.getDescriptionEN());
                    }
                    if (service.getMenu_url() != null) {
                        binding.foodService.btnMenu.setVisibility(View.VISIBLE);
    ...
    private void registerOrderAttempt() {
        ...
        Call<MessageResponse> call = api.registerOrder(new OrderRequest(user.getId(), selectedRoom, null, 3L, itemsAdapter.getOrderItems(), itemsAdapter.getTotalPrice(), comment));
